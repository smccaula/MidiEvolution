...
namespace as_wave
{
    class as_wave
    {
        ...
        static void Main(string[] args)
        {
         ...
         //   {
         //       GlobalVar.potentialDiff = GlobalVar.potentialDiff + (2 * Math.Abs(GlobalVar.leftmono[i]));  // worst is mirror
          //      GlobalVar.potentialDiff = GlobalVar.potentialDiff + ((128 * 256) + Math.Abs(GlobalVar.leftmono[i]));
        //   }

//            Console.WriteLine(" score " + GlobalVar.myScore.ToString() + " of " + GlobalVar.potentialDiff.ToString() +
...
//            CalculateNewWave();
    ...
    //            if (GlobalVar.myScore > -999999)
        ...
        static void GetStartsAndEnds()
        {
            long cycleInterval = 0;
            double fStart = 0.0;
            double fCenter = (sineInterval / 2);

            for (int frameX = 0; frameX < GlobalVar.framesThisRun; frameX++)
            {
                cycleInterval = 0;

                GlobalVar.frameFirstSample[frameX] = 0;
                GlobalVar.frameLastSample[frameX] = 0;

                if ((GlobalVar.levelFD[frameX] > 0) && (GlobalVar.levelAmplitude[frameX] > 0)) //dsm
                {
                    cycleInterval = Convert.ToInt64((samplesSecond / GlobalVar.levelFD[frameX]) / 2.0); //  half a cycle 
                }

                if (cycleInterval > 0)
                {
                    fStart = fCenter - (cycleInterval / 1) + GlobalVar.levelOffset[frameX]; // offset = 2 * cycle
                    GlobalVar.frameFirstSample[frameX] = Convert.ToInt32(fStart);
                    GlobalVar.frameLastSample[frameX] = Convert.ToInt32(fStart + cycleInterval);
                    if (GlobalVar.frameFirstSample[frameX] < 0)
                        GlobalVar.frameFirstSample[frameX] = 0;
                    if (GlobalVar.frameFirstSample[frameX] > GlobalVar.samples)
                        GlobalVar.frameFirstSample[frameX] = GlobalVar.samples;
                    if (GlobalVar.frameLastSample[frameX] < 0)
                        GlobalVar.frameLastSample[frameX] = 0;
                    if (GlobalVar.frameLastSample[frameX] > GlobalVar.samples)
                        GlobalVar.frameLastSample[frameX] = GlobalVar.samples;
                }
        ...
        static void CalcSamples()
        {
            ...
            for (int frameX = 0; frameX < GlobalVar.framesThisRun; frameX++)
            {
                ...
                {
                    calcSine = (TAU * GlobalVar.levelFD[frameX] * (double)(sineX)) / samplesSecond;
                    calcSine = (GlobalVar.levelAmplitude[frameX]) * Math.Sin(calcSine);

           //         Console.WriteLine(wx.ToString() + "  " + frameX.ToString());
                    sNDX = wx - GlobalVar.frameFirstSample[frameX];

                    try
                    {
                        GlobalVar.frameSamples[frameX, sNDX] = Convert.ToInt16(calcSine);
                    }
                    catch
                    {
                        if (calcSine > -1)
                            GlobalVar.frameSamples[frameX, sNDX] = ((128 * 256) - 1);

                        if (calcSine < 0)
                            GlobalVar.frameSamples[frameX, sNDX] = -(128 * 256) + 1;
                    }
                    sineX = sineX + GlobalVar.levelDirection[frameX];
                }
            }
        }
...
//            string fn = "";
//            fn = "sx" + Convert.ToString(GlobalVar.popMember);
//            File.WriteAllText(fn, bs);
   ...
   //             if (tempScore > GlobalVar.worstScore)
   //             {
   //                 GlobalVar.worstScore = tempScore;
   //                 GlobalVar.worstNDX = i;
   //             }
         ...
         //       potentialDiff = potentialDiff + ((128 * 256) + Math.Abs(GlobalVar.leftmono[i]));  // worst is extreme

//                GlobalVar.scoreAll++;
//                GlobalVar.scoreCount++;
